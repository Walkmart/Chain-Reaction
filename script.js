...
function showResultsScreen(failed = false) {
    ...
    markGameAsCompleted();  // <--- Mark the game as completed for the day
...
function shareResults(failed = false) {
  ...
  if (failed) {
    resultsText = `I reached word ${currentWordIndex + 1} out of ${wordChain.length} on Chain Words with a score of ${score}. Can you beat my progress?\nCheck out the game here: ${websiteLink}`;
  } else {
    resultsText = `I completed Chain Words in ${elapsedTime} seconds with a score of ${score}! Can you beat my time?\nCheck out the game here: ${websiteLink}`;
  }
