...
function checkIfPlayedToday() {
    ...
    // Check if the player has completed the game today
    if (lastPlayed === today && gameCompleted === 'true') {
        alert("You've already completed today's game. Come back tomorrow!");
        disableGame();  // Disable the game if they've already played and completed the game today
        return true;
    } else {
        return false;
    }
...
// Function to mark the game as completed in localStorage
function markGameAsCompleted() {
    const today = new Date().toLocaleDateString();  // Format the current date as "MM/DD/YYYY"
    localStorage.setItem('lastPlayedDate', today);  // Store today's date
    localStorage.setItem('gameCompleted', 'true');  // Mark the game as completed
}
...
function showResultsScreen(failed = false) {
    ...
    // Mark the game as completed to prevent replay
    markGameAsCompleted();
}
