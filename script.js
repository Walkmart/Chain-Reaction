...
// Function to mark the game as completed and prevent further play
function markGameAsCompleted() {
    const today = new Date().toLocaleDateString();  // Get today's date in a readable format
    localStorage.setItem('gameCompletedToday', today);  // Store the completion date in localStorage
    disableGame();  // Disable the game immediately
}

// Function to check if the game was completed today
function checkIfGameCompletedToday() {
    const today = new Date().toLocaleDateString();  // Get today's date
    const gameCompletedDate = localStorage.getItem('gameCompletedToday');  // Get the date when the game was last completed

    return gameCompletedDate === today;  // Check if today's date matches the stored date
}

// Disable game by disabling input elements
function disableGame() {
    document.getElementById('guess-input').disabled = true;
    document.getElementById('submit-guess').disabled = true;
    document.getElementById('hint-btn').disabled = true;
    document.querySelector('.game-container').style.opacity = "0.5";  // Dim the game
    console.log("Game has been disabled because it was completed today.");
}

// Check if the game has already been completed today when the page loads
document.addEventListener('DOMContentLoaded', (event) => {
    if (checkIfGameCompletedToday()) {
        disableGame();  // If completed, disable the game immediately
        alert("You've already completed today's game. Come back tomorrow!");
        return;
    }

    // Normal game setup if they haven't played or completed it yet
...
// Function to trigger when the game finishes (win or lose)
function gameFinished() {
    markGameAsCompleted();  // Mark the game as finished and lock it
    showResultsScreen();  // Show the results screen
}

// Example of triggering the game finish logic after completing the word chain
function moveToNextWord() {
  currentWordIndex++;

  // Check if the entire chain is completed
  if (currentWordIndex >= wordChain.length) {
    feedback.innerText = "Congratulations! You've completed the word chain!";
    score += bonusPoints;
    scoreDisplay.innerText = score;
    gameFinished();  // Mark game as finished and lock it
  } else {
    // Set up the next word logic
...
// Disable game on failure as well
function checkGuess() {
    ...
    for (let i = 0; i < wordLength; i++) {
        const tile = tiles[i];
        tile.innerText = userGuess[i].toUpperCase();
...
document.addEventListener('DOMContentLoaded', (event) => {
    if (checkIfPlayedToday()) {
        // If they've already played, stop here
        console.log("User has already played today. Game disabled.");
        return;
    }

    // Normal game setup if they haven't played today
    createAllGrids();  // Setup the word grids
    attemptCountDisplay.innerText = attemptsLeft;
    scoreDisplay.innerText = score;
});
