...
// Function to check if the game has been completed today
function checkIfGameCompletedToday() {
    const today = new Date().toLocaleDateString();  // Format the current date as "MM/DD/YYYY"
    const lastCompleted = localStorage.getItem('gameCompletedToday');

    // Compare the saved date with today's date
    if (lastCompleted === today) {
        alert("You've already completed the game today. Come back tomorrow!");
        disableGame();  // Disable the game but still allow sharing
        return true;
    }
    return false;
}

// Function to mark the game as completed for the day
function markGameAsCompleted() {
    const today = new Date().toLocaleDateString();
    localStorage.setItem('gameCompletedToday', today);  // Save today's date in localStorage
    disableGame();  // Disable the game
}
...
function disableGame() {
    ...
    document.querySelector('.game-container').style.opacity = "0.5";  // Dim the game board for clarity
    console.log("Game has been disabled because you've already played today.");
}

// Start game logic
document.addEventListener('DOMContentLoaded', (event) => {
    if (checkIfGameCompletedToday()) {
        // If they've already played and completed the game today, stop here
        console.log("User has already completed the game today. Game disabled.");
        return;
    }

    // Normal game setup if they haven't completed the game today
...
function showResultsScreen(failed = false) {
    ...
    const minutes = Math.floor(elapsedTime / 60);
    const seconds = elapsedTime % 60;
    const formattedTime = `${minutes}m ${seconds}s`;  // Format time in "minutes:seconds"
    ...
    finalTimeDisplay.innerText = `Time: ${formattedTime}`;  // Display formatted time
...
function shareResults(failed = false) {
    ...
    if (failed) {
        resultsText = `I reached word ${currentWordIndex + 1} out of ${wordChain.length} on Compundle with a score of ${score}. Can you beat my progress?\nCheck out the game here: ${websiteLink}\nTime: ${formattedTime}`;
    } else {
        resultsText = `I completed Compundle in ${formattedTime} with a score of ${score}! Can you beat my time?\nCheck out the game here: ${websiteLink}`;
    }
